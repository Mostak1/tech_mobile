      ...
      axios.get(`/products/serials/search?search=${encodeURIComponent(this.searchSerialQuery.trim())}`)
        .then(response => {
          if (response.data.success) {
            const { product_id, serial_id } = response.data;
            this.searchSerialQuery = '';
            this.$router.push({
              path: `/app/products/detail/${product_id}`,
              query: { search_serial_id: serial_id }
            }).catch(() => {
              if (this.$route.params.id == product_id) {
                this.$router.replace({
                  query: { ...this.$route.query, search_serial_id: serial_id, _t: Date.now() }
                });
              }
            });
          } else {
            this.$root.$bvToast.toast('Serial/IMEI not found', { title: 'Warning', variant: 'warning', solid: true });
          }
        })
        .catch(error => {
