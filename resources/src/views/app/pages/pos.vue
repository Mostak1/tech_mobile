        ...
        <div class="pos-shell-cart-scroll" style="overflow-y: auto; overflow-x: hidden; padding: 8px; min-height: 0; height: 100%;">
          ...
          <div v-else style="display: flex; flex-direction: column;">
            <div
              ...
              <!-- Serial tracking panel (full-width, only for serial-tracked items) -->
              <div v-if="item.enable_serial_tracking" style="grid-column: 1 / -1; margin-top: 6px; padding: 8px; background: linear-gradient(135deg, #f0fdf4 0%, #ecfdf5 100%); border: 1px solid #d1fae5; border-left: 3px solid #10b981; border-radius: 8px;">
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px;">
                  <div style="font-size: 11px; font-weight: 600; color: #065f46; display: inline-flex; align-items: center; gap: 4px;">
                    <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="12" rx="2"/><path d="M6 12h.01M10 12h.01M14 12h.01M18 12h.01"/></svg>
                    <span>{{ $t('Serial_Numbers') || 'Serial Numbers' }}</span>
                    <span :style="{ fontSize: '10px', fontFamily: '\'JetBrains Mono\', monospace', fontWeight: 700, padding: '1px 6px', borderRadius: '4px', background: (item.serial_numbers || []).length === Math.floor(item.quantity) ? '#d1fae5' : '#fecaca', color: (item.serial_numbers || []).length === Math.floor(item.quantity) ? '#065f46' : '#991b1b' }">
                      {{ (item.serial_numbers || []).length }} / {{ Math.floor(item.quantity) }}
                    </span>
                  </div>
                </div>
                <textarea
                  ...
                  @input="evt => onPosSerialInput(item, evt.target.value)"
                  :placeholder="$t('Scan_or_paste_serial_numbers_here') || 'Scan/paste serial numbers (one per line)...'"
                  style="width: 100%; padding: 6px 8px; border: 1px solid #d1fae5; border-radius: 6px; background: #ffffff; font-size: 11px; font-family: 'JetBrains Mono', monospace; color: #1f1f2c; outline: none; resize: vertical; min-height: 40px;"
                ></textarea>
                <div
                  v-if="(item.serial_numbers || []).length !== Math.floor(item.quantity)"
                  style="margin-top: 4px; font-size: 10px; color: #dc2626; display: flex; align-items: center; gap: 4px;"
                >
                  <svg viewBox="0 0 20 20" fill="currentColor" style="width: 12px; height: 12px; flex-shrink: 0;"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                  <span>{{ $t('Serial_Qty_Mismatch') || 'Serial count must match quantity' }}</span>
                </div>
              </div>
            </div>
          </div>
  ...
  <validation-observer ref="Update_Detail">
    ...
    <b-form @submit.prevent="submit_Update_Detail">
        <b-row>
          ...
          <b-col lg="12" md="12" sm="12" v-show="detail.enable_serial_tracking && !detailLoading">
            <b-form-group>
              <template #label>
                <div style="display: flex; align-items: center; gap: 6px;">
                  <span style="font-weight: 600;">{{ $t('Serial_Numbers') || 'Serial Numbers' }}</span>
                  <span :style="{ fontSize: '11px', fontFamily: '\'JetBrains Mono\', monospace', fontWeight: 700, padding: '2px 8px', borderRadius: '4px', background: (detail.serial_numbers || []).length === Math.floor(detail.quantity) ? '#d1fae5' : '#fecaca', color: (detail.serial_numbers || []).length === Math.floor(detail.quantity) ? '#065f46' : '#991b1b' }">
              ...
              <b-form-textarea
                ...
                @input="val => onPosSerialInput(detail, val)"
                :placeholder="$t('Scan_or_paste_serial_numbers_here') || 'Scan/paste serial numbers (one per line or comma-separated)...'"
              ...
              >
                {{ $t('Serial_Qty_Mismatch') || 'Serial numbers count must match the product quantity.' }}
              </div>
            </b-form-group>
          </b-col>
  ...
  methods: {
    ...
    async processPayment() {
      ...
      if (error) {
        this.paymentProcessing = false;
        NProgress.done();
        this.makeToast("danger", error.message || this.$t("InvalidData"), this.$t("Failed"));
      } else {
    ...
    async Get_Product_Details(product_id, variant_id, sourceProduct = null) {
      ...
      const applyDetail = (data) => {
        ...
        this.product.enable_serial_tracking = !!data.enable_serial_tracking;
      ...
      const buildDetailFromSource = () => {
        ...
        const detail = {
          ...
          enable_serial_tracking: !!p.enable_serial_tracking,
          serial_numbers: [],
    ...
    add_product(code) {
      ...
      // Serial tracking: auto-open edit modal for serial-tracked items
      if (newItem.enable_serial_tracking && !newItem.is_imei) {
        this.Modal_Updat_Detail(newItem);
      }
    ...
    Modal_Updat_Detail(detail) {
      ...
      this.get_units(detail.product_id)
        ...
        .finally(() => {
          ...
          this.detail.enable_serial_tracking = detail.enable_serial_tracking || false;
          this.$set(this.detail, 'serial_numbers', detail.serial_numbers || []);
    ...
    submit_Update_Detail() {
      for (var i = 0; i < this.details.length; i++) {
        if (this.details[i].detail_id === this.detail.detail_id) {
          ...
          this.details[i].serial_numbers = this.detail.serial_numbers || [];
    ...
    // Parse serial number textarea input for serial-tracked items
    onPosSerialInput(item, val) {
      const serials = val
        .split(/[\n,]+/)
        .map(s => s.trim())
        .filter(s => s.length > 0);
    ...
    GetElementsPos() {
      axios
        ...
        .then(response => {
          ...
          if ((!this.sale.warehouse_id || !this.warehouses.find(w => w.id === this.sale.warehouse_id)) && this.warehouses.length > 0) {
