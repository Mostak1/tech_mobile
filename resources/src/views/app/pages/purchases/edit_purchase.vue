                  ...
                  <div class="table-responsive">
                    <table class="table table-hover">
                      ...
                      <tbody>
                        ...
                        <tr v-if="detail.enable_serial_tracking && detail.no_unit !== 0 && detail.del !== 1" :key="'serials-'+detail.detail_id" :style="{ background: 'transparent' }">
                          <td colspan="9" :style="{ padding: '0 8px 16px 8px', border: 'none' }">
                            <div
                              :style="{
                            ...
                            >
                              <div class="d-flex align-items-center justify-content-between mb-2">
                                <div class="d-flex align-items-center gap-2">
                                  <lucide-icon name="list" :style="{ color: '#10b981', fontSize: '18px' }" />
                                  <span :style="{ fontWeight: '700', fontSize: '13px', color: '#065f46' }">
                                    {{ $t('Serial_IMEI_Numbers') || 'Serial/IMEI Numbers' }}
                                  </span>
                                  <span class="badge badge-success ml-2">
                                    {{ (detail.serial_numbers || []).length }} / {{ detail.quantity }}
                                  </span>
                                </div>
                              </div>
                ...
                <b-col md="12">
                  <b-form-group>
                    <b-button
                      ...
                      :disabled="SubmitProcessing || hasBatchValidationErrors || hasSerialValidationErrors"
                      :title="hasBatchValidationErrors ? $t('Batch_Qty_Mismatch') : (hasSerialValidationErrors ? $t('Serial_Qty_Mismatch') : '')"
                    ...
                    <div v-if="hasSerialValidationErrors" class="text-danger mt-2" style="font-size: 13px;">
                      <lucide-icon name="alert-triangle" style="vertical-align: middle; margin-right: 4px;" />
                      <template v-if="firstSerialErrorDetail && (!firstSerialErrorDetail.serial_numbers || firstSerialErrorDetail.serial_numbers.length === 0)">
                        {{ $t('Serial_Required_For_Item') || 'Add serial numbers for' }}: <strong>{{ firstSerialErrorDetail.name }}</strong>
                      </template>
                      <template v-else>
                        {{ $t('Serial_Qty_Mismatch') || 'Serial numbers count mismatch for' }}: <strong>{{ firstSerialErrorDetail.name }}</strong>
                      </template>
                    </div>
...
export default {
  ...
  computed: {
    ...
    activeSerialTrackedDetails() {
      if (!Array.isArray(this.details)) return [];
      return this.details.filter(d =>
        d && d.enable_serial_tracking && d.no_unit !== 0 && d.del !== 1
      );
    },

    hasSerialValidationErrors() {
      return this.activeSerialTrackedDetails.some(d => {
        if (!Array.isArray(d.serial_numbers) || d.serial_numbers.length === 0) return true;
        return d.serial_numbers.length !== Math.floor(d.quantity);
      });
    },

    firstSerialErrorDetail() {
      return this.activeSerialTrackedDetails.find(d => {
        if (!Array.isArray(d.serial_numbers) || d.serial_numbers.length === 0) return true;
        return d.serial_numbers.length !== Math.floor(d.quantity);
      }) || null;
    }
  ...
  methods: {
    ...
    Submit_Purchase() {
      ...
      if (this.hasSerialValidationErrors) {
        ...
        const missing = d && (!Array.isArray(d.serial_numbers) || d.serial_numbers.length === 0);
        const msg = missing
          ? `${this.$t('Serial_Required_For_Item') || 'Add serial numbers for'}: ${d ? d.name : ''}`
          : `${this.$t('Serial_Qty_Mismatch') || 'Serial numbers count mismatch for'}: ${d ? d.name : ''}`;
        this.makeToast("danger", msg, this.$t("Failed"));
        return;
      }
