          ...
          <b-col lg="12" md="12" sm="12">
            <b-card>
              <b-row>
                ...
                <b-col md="12">
                  <b-form-group>
                     <b-button variant="primary" :disabled="SubmitProcessing || hasBatchValidationErrors || hasSerialValidationErrors" @click="Submit_Adjustment"><lucide-icon class="me-2 font-weight-bold" name="check" /> {{$t('submit')}}</b-button>
  ...
  computed: {
    ...
    hasSerialValidationErrors() {
      ...
      for (const d of this.details) {
        ...
        const reqCount = Math.floor(d.quantity);
        const currentCount = (d.serial_numbers || []).length;
        if (currentCount !== reqCount) return true;
      }
      return false;
    }
  },
  ...
  methods: {
   ...
   //------------------------------------ Get Products By Warehouse -------------------------\\
    ...
    add_product() {
      ...
      if (last && last.enable_serial_tracking) {
    ...
    fetch_serials_for_detail(detail) {
      ...
      if (detail.type !== 'sub') {
        this.$set(detail, "available_serials", []);
        this.$set(detail, "serials_loading", false);
        return;
      }
    ...
    toggle_serial_number(detail, sn) {
      ...
      if (idx > -1) {
        ...
        if (detail.serial_numbers.length < Math.floor(detail.quantity)) {
          ...
          this.makeToast("warning", "Cannot select more serial numbers than product quantity", "Warning");
        }
      }
      this.$forceUpdate();
    },
    ...
    onTypeChange(detail) {
      this.$set(detail, "serial_numbers", []);
      this.fetch_serials_for_detail(detail);
    ...
    buildSubmitDetails() {
      return (this.details || []).map(d => {
        ...
        if (d.enable_serial_tracking && Array.isArray(d.serial_numbers)) {
          out.serial_numbers = d.serial_numbers;
        } else {
          delete out.serial_numbers;
        }
        return out;
      });
    },
    ...
    verifiedForm() {
      if (this.details.length <= 0) {
        ...
        for (const d of this.details) {
          if (d.enable_serial_tracking) {
            const reqCount = Math.floor(d.quantity);
            const currentCount = (d.serial_numbers || []).length;
            if (currentCount !== reqCount) {
              this.makeToast("danger", `Product ${d.name} requires exactly ${reqCount} serial numbers, but ${currentCount} were provided.`, "Failed");
              return false;
            }
          }
        }
      ...
      axios
        ...
        .then(response => {
          ...
          if (this.warehouses.length > 0) {
            this.adjustment.warehouse_id = this.warehouses[0].id;
            this.Selected_Warehouse(this.adjustment.warehouse_id);
          }
