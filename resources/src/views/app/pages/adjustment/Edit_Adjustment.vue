                  ...
                  <div class="table-responsive">
                    <table class="table table-hover">
                      ...
                      <tbody>
                        ...
                        <tr v-if="detail.enable_serial_tracking && detail.del !== 1" :key="'serials-'+detail.detail_id" style="background: transparent;">
                          <td colspan="7" style="padding: 0; border-top: 0;">
...
export default {
  ...
  computed: {
    ...
    hasSerialValidationErrors() {
      if (!Array.isArray(this.details)) return false;
      for (const d of this.details) {
        if (!d || !d.enable_serial_tracking || d.del === 1) continue;
  ...
  methods: {
    ...
    verifiedForm() {
      if (this.details.length <= 0) {
        ...
        for (const d of this.details) {
          if (d.enable_serial_tracking && d.del !== 1) {
            const reqCount = Math.floor(d.quantity);
            const currentCount = (d.serial_numbers || []).length;
