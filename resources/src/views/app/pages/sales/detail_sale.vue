    ...
    <b-card v-if="!isLoading" class="print-card">
      ...
      <div class="invoice" id="print_Invoice">
        <div class="invoice-print">
          ...
          <!-- Payments Section -->
          <div v-if="payments && payments.length > 0" class="invoice-payments-section mt-4 mb-4">
            <div class="invoice-box-header" style="background: #1a56db; color: #ffffff; padding: 5px 10px; font-weight: bold; border-radius: 4px 4px 0 0; text-transform: uppercase;">Payments</div>
            <table class="invoice-products-table" style="margin-top: 0;">
              <thead>
                <tr class="invoice-products-header">
                  <th class="invoice-products-th-left">Date</th>
                  <th class="invoice-products-th-left">Payment Ref</th>
                  <th class="invoice-products-th-left">Payment Method</th>
                  <th class="invoice-products-th-right" style="border-right: none;">Amount</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(p, index) in payments" :key="p.id" :class="{'invoice-products-row-even': index % 2 === 1}">
                  <td style="padding: 5px; font-size: 8.5pt; text-align: left;">{{ p.date }}</td>
                  <td style="padding: 5px; font-size: 8.5pt; text-align: left;">{{ p.Ref }}</td>
                  <td style="padding: 5px; font-size: 8.5pt; text-align: left;">{{ p.payment_method ? p.payment_method.name : '---' }}</td>
                  <td style="padding: 5px; font-size: 9pt; font-weight: bold; text-align: right; color: #1a56db;">{{ formatPriceWithSymbol(currentUser.currency, p.montant, 2) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
