            ...
            <b-card>
              <b-row>
                  ...
                  <div class="table-responsive">
                    <table class="table table-hover">
                      ...
                      <tbody>
                        ...
                        <!-- Serial numbers input row for serial-tracked products -->
                        <tr v-if="detail.enable_serial_tracking" :key="'serials-'+detail.detail_id" style="background: transparent;">
                          <td colspan="9" style="padding: 0; border-top: 0;">
                            <div style="margin: 6px 8px 14px 8px; border: 1px solid #d1fae5; border-radius: 10px; overflow: hidden; background: linear-gradient(180deg, #f0fdf4 0%, #ffffff 100%);">
                              <div style="display: flex; align-items: center; justify-content: space-between; padding: 8px 14px; background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: #fff; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.3px;">
                                <div style="display: flex; align-items: center; gap: 8px;">
                                  <lucide-icon name="list" />
                                  <span>{{ $t('Serial_IMEI_Numbers') || 'Serial/IMEI Numbers' }}</span>
                                  <span style="background: rgba(255,255,255,0.22); padding: 1px 8px; border-radius: 10px; font-size: 10px; font-weight: 600;">
                                    {{ (detail.serial_numbers || []).length }} / {{ Math.floor(detail.quantity) }}
                                  </span>
                                </div>
                              </div>

                              <div v-if="detail.serials_loading" style="padding: 10px 14px; text-align: center; color: #6b7280; font-size: 12px;">
                                <div class="spinner sm spinner-primary" style="display: inline-block; margin-right: 8px;"></div>
                                {{ $t('Loading') || 'Loading...' }}
                              </div>

                              <div v-else style="padding: 14px 18px;">
                                <b-form-group
                                  :label="($t('Enter_Serial_Numbers_Placeholder') || 'Enter Serial Numbers (one per line or separated by comma)')"
                                  label-class="font-weight-bold"
                                  :style="{ fontSize: '12px', color: '#1e293b', marginBottom: '8px' }"
                                >
                                  <b-form-textarea
                                    rows="3"
                                    :value="(detail.serial_numbers || []).join('\n')"
                                    @input="val => onSerialInput(detail, val)"
                                    :placeholder="($t('Scan_or_paste_serial_numbers_here') || 'Scan/paste serial numbers here...')"
                                    :style="{ fontSize: '13px', fontFamily: 'monospace' }"
                                  />
                                </b-form-group>

                                <div v-if="detail.available_serials && detail.available_serials.length" class="mt-2">
                                  <div style="font-size: 11px; font-weight: bold; color: #4b5563; margin-bottom: 4px;">
                                    {{ $t('Available_Serials_Hint') || 'Available Serials in this Warehouse (Click to select/add):' }}
                                  </div>
                                  <div class="d-flex flex-wrap gap-1" style="max-height: 100px; overflow-y: auto; padding: 4px; background: #fff; border: 1px solid #e5e7eb; border-radius: 6px;">
                                    <span
                                      v-for="sn in detail.available_serials"
                                      :key="sn"
                                      @click="toggle_serial_number(detail, sn)"
                                      :style="{
                                        cursor: 'pointer',
                                        fontSize: '11px',
                                        padding: '2px 6px',
                                        borderRadius: '4px',
                                        border: '1px solid',
                                        backgroundColor: (detail.serial_numbers || []).includes(sn) ? '#ecfdf5' : '#f3f4f6',
                                        borderColor: (detail.serial_numbers || []).includes(sn) ? '#10b981' : '#e5e7eb',
                                        color: (detail.serial_numbers || []).includes(sn) ? '#065f46' : '#374151',
                                        marginRight: '4px',
                                        marginBottom: '4px',
                                ...
                                <div v-else class="text-warning mt-2" style="font-size: 11px;">
                                  <lucide-icon name="alert-triangle" style="font-size: 13px; margin-right: 4px;" />
                                  {{ $t('No_Available_Serials') || 'No available serial numbers found in the selected warehouse.' }}
                                </div>

                                <div
                                  v-if="(detail.serial_numbers || []).length !== Math.floor(detail.quantity)"
                                  class="text-danger mt-2"
                                  style="font-size: 12px; display: flex; align-items: center; gap: 6px;"
                                >
                                  <lucide-icon name="alert-triangle" style="font-size: 14px; margin-right: 4px;" />
                                  <span>{{ $t('Serial_Qty_Mismatch') || 'Serial numbers count must match the product quantity exactly.' }}</span>
                                </div>
                              </div>
                            </div>
                          </td>
                ...
                <b-col md="12" v-if="sale.statut === 'completed' && hasBatchValidationErrors">
                 ...
                 <b-col md="12" v-if="hasSerialValidationErrors">
                   <div class="alert alert-warning mt-2" style="font-size: 13px; font-weight: 600;">
                     <lucide-icon class="me-1" name="info" />
...
export default {
  ...
  computed: {
    ...
    hasSerialValidationErrors() {
      if (!Array.isArray(this.details)) return false;
      for (const d of this.details) {
        if (!d || !d.enable_serial_tracking) continue;
        const serials = Array.isArray(d.serial_numbers) ? d.serial_numbers : [];
        if (serials.length !== Math.floor(d.quantity)) return true;
        if (new Set(serials).size !== serials.length) return true;
      }
      return false;
    },
    ...
    firstSerialErrorMessage() {
      ...
      for (const d of this.details) {
        ...
        const serials = Array.isArray(d.serial_numbers) ? d.serial_numbers : [];
        const label = d.name || d.code || "";
        if (serials.length !== Math.floor(d.quantity)) {
          return `${this.$t('Serial_Qty_Mismatch') || 'Serial numbers count must match the product quantity exactly.'} (${serials.length} / ${Math.floor(d.quantity)}) — ${label}`;
        }
        if (new Set(serials).size !== serials.length) {
          return `${this.$t('Duplicate_Serial_Selected') || 'Duplicate serial numbers are not allowed'} — ${label}`;
        }
      }
      return "";
    }
  ...
  methods: {
    ...
    Submit_Sale() {
      this.$refs.edit_sale.validate().then(success => {
        if (!success) {
          ...
          if (this.hasSerialValidationErrors) {
    ...
    Selected_Warehouse(value) {
      ...
      if (Array.isArray(this.details)) {
        for (const d of this.details) {
          ...
          if (d && d.enable_serial_tracking) {
            this.$set(d, "serial_numbers", []);
            this.fetch_serials_for_detail(d);
          }
        }
      }
    },
    ...
    prefillSerialsForTrackedDetails() {
      if (!Array.isArray(this.details)) return;
      for (const d of this.details) {
        if (!d || !d.enable_serial_tracking) continue;
        ...
        this.$set(d, "available_serials", []);
        this.$set(d, "serials_loading", false);
        this.fetch_serials_for_detail(d);
      }
    },

    fetch_serials_for_detail(detail) {
      if (!detail) return;
      if (!("serials_loading" in detail)) this.$set(detail, "serials_loading", false);
      if (!("available_serials" in detail)) this.$set(detail, "available_serials", []);
      if (!Array.isArray(detail.serial_numbers)) this.$set(detail, "serial_numbers", []);

      if (!detail.enable_serial_tracking) {
        this.$set(detail, "serials_loading", false);
        return;
      }
      const wid = this.sale && this.sale.warehouse_id;
      ...
      this.$set(detail, "serials_loading", true);
      const params = {
        product_id: detail.product_id,
        warehouse_id: wid,
      };
      if (detail.product_variant_id != null && detail.product_variant_id !== "") {
        params.variant_id = detail.product_variant_id;
      }
      
      axios
        .get('products/serials/available', { params })
        .then(response => {
          let list = Array.isArray(response.data) ? response.data : [];
          
          if (Array.isArray(detail.serial_numbers)) {
            detail.serial_numbers.forEach(sn => {
              if (sn && !list.includes(sn)) {
                list.push(sn);
              }
            });
          }
          
          this.$set(detail, "available_serials", list);
          this.$set(detail, "serials_loading", false);
        })
        .catch(error => {
          this.$set(detail, "serials_loading", false);
          console.error("Failed fetching serial numbers", error);
        });
    },

    onSerialInput(detail, val) {
      if (!val) {
        this.$set(detail, 'serial_numbers', []);
      } else {
        const serials = val.split(/[\n,]+/).map(s => s.trim()).filter(Boolean);
        this.$set(detail, 'serial_numbers', serials);
      }
    },

    toggle_serial_number(detail, sn) {
      if (!Array.isArray(detail.serial_numbers)) {
        this.$set(detail, 'serial_numbers', []);
      }
      const idx = detail.serial_numbers.indexOf(sn);
      if (idx > -1) {
        detail.serial_numbers.splice(idx, 1);
      } else {
        if (detail.serial_numbers.length < Math.floor(detail.quantity)) {
          detail.serial_numbers.push(sn);
        } else {
          this.makeToast("warning", this.$t("Serial_Qty_Limit_Reached") || "Cannot select more serial numbers than product quantity", this.$t("Warning") || "Warning");
        }
      }
      this.$forceUpdate();
    },
    ...
    Get_Product_Details(product_id, variant_id) {
      ...
      axios.get(url).then(response => {
        ...
        this.product.enable_serial_tracking = !!response.data.enable_serial_tracking;
        this.product.serial_numbers = [];
        this.product.available_serials = [];
        this.product.serials_loading = false;
        ...
        if (this.product.enable_serial_tracking) {
          const last = this.details[this.details.length - 1];
          if (last) {
            this.fetch_serials_for_detail(last);
          }
        }
      });
    },
          ...
          if (this.sale.client_id) {
            axios
              ...
              .finally(() => {
                ...
                this.prefillSerialsForTrackedDetails();
                this.Calcul_Total();
