...
class AdjustmentController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        if ($serialService->isSupported()) {
            $serialErrors = $serialService->validateAdjustment($request['details'], (int) $request->warehouse_id);
            if (!empty($serialErrors)) {
                return response()->json([
                    'success' => false,
                    'message' => implode(' ', $serialErrors),
                ], 422);
            }
        }

        \DB::transaction(function () use ($request, $serialService) {
            ...
            foreach ($data as $key => $value) {
                $persistedDetails[$key] = AdjustmentDetail::create([
                    ...
                    'type' => $value['type'],
                    'imei_number' => $value['serial_numbers'] ?? $value['imei_number'] ?? null,
                ]);
            ...
            // Serial tracking: apply adjustment serials
            if ($serialService->isSupported()) {
                $serialService->applyForAdjustment(
                    $order,
                    array_values($data),
                    $persistedDetails
                );
    ...
    public function update(Request $request, $id)
    {
        ...
        */
        ...
        \DB::transaction(function () use ($request, $id, $current_adjustment, $serialService) {
            ...
            // Serial tracking: reverse old adjustment serials
            if ($serialService->isSupported()) {
                $serialService->reverseForAdjustmentDetails($old_adjustment_details);
            }
            ...
            foreach ($new_adjustment_details as $key => $product_detail) {
                ...
                $orderDetails['imei_number'] = $product_detail['serial_numbers'] ?? $product_detail['imei_number'] ?? null;
            ...
            // Serial tracking: re-apply adjustment serials
            if ($serialService->isSupported()) {
                $alignedInput = [];
                $alignedPersisted = [];
                foreach ($new_adjustment_details as $key => $product_detail) {
    ...
    public function destroy(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($id, $request) {
            ...
            */
            ...
            // Serial tracking: reverse adjustment serials on delete
            $serialService = app(\App\Services\SerialTrackingService::class);
            if ($serialService->isSupported()) {
                $serialService->reverseForAdjustmentDetails($old_adjustment_details);
            }
    ...
    public function edit(Request $request, $id)
    {
        ...
        */
        ...
        // Fetch serial numbers associated with this adjustment
        $histories = \App\Models\SerialNumberHistory::where('type', 'adjustment')
            ->where('reference_id', $Adjustment_data->id)
            ->with('serialNumber')
            ->get();
        $serialNumbersByProduct = [];
        foreach ($histories as $history) {
            $sn = $history->serialNumber;
            if ($sn) {
                $key = $sn->product_id . '-' . ($sn->variation_id ?: '0');
                $serialNumbersByProduct[$key][] = $sn->serial_no;
            }
        }
        ...
        foreach ($Adjustment_data['details'] as $detail) {
            ...
            $prodKey = $detail->product_id . '-' . ($detail->product_variant_id ?: '0');
            $data['is_batch_tracked'] = (bool) ($detail['product']['is_batch_tracked'] ?? false);
            $data['batches'] = $batchesByDetail[(int) $detail->id] ?? [];
            $data['enable_serial_tracking'] = (bool) ($detail['product']['enable_serial_tracking'] ?? false);
            $data['serial_numbers'] = $serialNumbersByProduct[$prodKey] ?? [];
