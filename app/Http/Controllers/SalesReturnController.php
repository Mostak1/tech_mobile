...
class SalesReturnController extends BaseController
{
    ...
    public function store(request $request)
    {
        ...
        if ($serialService->isSupported()) {
            $serialErrors = $serialService->validateSaleReturn($request['details'], (int) $request->warehouse_id);
        ...
        \DB::transaction(function () use ($request, $serialService) {
            ...
            if ($order->statut == 'received') {
                ...
                // Serial tracking: apply sale return serials
                if ($serialService->isSupported()) {
                    $serialService->applyForSaleReturn(
                        $order,
                        array_values($data),
                        $persistedDetails
                    );
    ...
    public function update(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($request, $id, $serialService) {
            ...
            */
            ...
            // Serial tracking: reverse old sale return serials
            if ($serialService->isSupported() && $current_SaleReturn->statut == 'received') {
                $serialService->reverseForSaleReturnDetails($old_return_details);
            }
            ...
            // Serial tracking: re-apply sale return serials
            if ($serialService->isSupported() && $request['statut'] == 'received') {
                $alignedInput = [];
                $alignedPersisted = [];
                foreach ($new_return_details as $key => $product_detail) {
                    if (isset($newPersistedDetails[$key])) {
    ...
    public function destroy(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($id, $request) {
            ...
            */
            ...
            // Serial tracking: reverse sales return serials on delete
            $serialService = app(\App\Services\SerialTrackingService::class);
            if ($serialService->isSupported() && $current_SaleReturn->statut == 'received') {
                $serialService->reverseForSaleReturnDetails($old_return_details);
            }
