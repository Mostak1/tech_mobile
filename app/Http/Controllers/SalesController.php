...
class SalesController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        $saleCostsMap = $serialService->calculateSalePurchaseCosts($Sales);
        ...
        foreach ($Sales as $Sale) {
            ...
            $costTotal = $saleCostsMap[$Sale->id] ?? 0.0;
    ...
    public function store(Request $request)
    {
        ...
        if ($serialService->isSupported()) {
            $serialErrors = $serialService->validateSale($request['details'], $request->warehouse_id);
        ...
        $sale = \DB::transaction(function () use ($request, $serialService) {
            $helpers = new helpers;
            $order = new Sale;
            ...
            if ($serialService->isSupported() && $order->statut === 'completed') {
                $persistedDetails = SaleDetail::where('sale_id', $order->id)
                    ->orderBy('id', 'asc')
                    ->get();
                $serialService->applyForSale($order, $data, $persistedDetails);
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        $serialService = app(\App\Services\SerialTrackingService::class);
        if ($serialService->isSupported()) {
            $serialErrors = $serialService->validateSale($request['details'], $request->warehouse_id, $id);
            if (!empty($serialErrors)) {
                return response()->json([
                    'success' => false,
                    'message' => implode(' ', $serialErrors),
        ...
        $sale = \DB::transaction(function () use ($request, $id, $serialService) {
            ...
            */
            ...
            if (SaleReturn::where('sale_id', $id)->where('deleted_at', '=', null)->exists()) {
                ...
                // Serial tracking: re-apply serials
                if ($serialService->isSupported() && $request['statut'] == 'completed') {
                    $current_Sale->warehouse_id = (int) $request->warehouse_id;
                    $serialService->applyForSale(
                        $current_Sale,
                        $new_sale_details,
                        $newPersistedDetails
                    );
    ...
    public function show(Request $request, $id)
    {
        ...
        $payments = \App\Models\PaymentSale::with('payment_method')
            ->where('sale_id', $id)
            ->where('deleted_at', '=', null)
            ->get();
