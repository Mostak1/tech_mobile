...
class ServiceJobController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $serialErrors = $this->validateServiceJobSerials($validated['items'] ?? []);
        ...
        $job = DB::transaction(function () use ($validated) {
            ...
            $trackingService = app(\App\Services\SerialTrackingService::class);
            $trackingService->applyForServiceJob($job);
        ...
        $items = $job->items()
            ...
            ->map(function (ServiceJobItem $row) {
                return [
                    ...
                    'notes' => $row->notes,
                    'serial_numbers' => $row->product_id ? ProductSerialNumber::where('service_job_id', $row->service_job_id)
                        ->where('product_id', $row->product_id)
                        ->when($row->product_variant_id, function ($q) use ($row) {
                            $q->where('variation_id', $row->product_variant_id);
                        })
                        ->pluck('serial_no')
                        ->toArray() : [],
                ];
    ...
    public function update(Request $request, $id)
    {
        ...
        $serialErrors = $this->validateServiceJobSerials($validated['items'] ?? [], $id);
        if (!empty($serialErrors)) {
            return response()->json([
                'success' => false,
                'message' => 'Serial number validation failed.',
    ...
    public function destroy(Request $request, $id)
    {
        ...
        DB::transaction(function () use ($job) {
            $trackingService = app(\App\Services\SerialTrackingService::class);
            $trackingService->reverseForServiceJob($job);
    ...
    protected function validatePayload(Request $request): array
    {
        return $request->validate([
            ...
            'items.*.serial_numbers.*' => 'string|max:191',
        ]);
    }

    protected function validateServiceJobSerials(array $items, ?int $serviceJobId = null): array
    {
        $errors = [];
        $service = app(\App\Services\SerialTrackingService::class);
        if (!$service->isSupported()) {
            return $errors;
        }

        foreach ($items as $item) {
            if (isset($item['type']) && $item['type'] === 'part' && isset($item['product_id'])) {
                if ($service->isProductTracked($item['product_id'])) {
                    $qty = (float) ($item['quantity'] ?? 0);
                    $serials = $service->parseSerials($item['serial_numbers'] ?? '');
                    
                    if (count($serials) !== (int) $qty) {
                        $errors[] = "Product " . ($item['description'] ?? "ID {$item['product_id']}") . ": Quantity is $qty, but " . count($serials) . " serial numbers were selected.";
                        continue;
                    }

                    if (count(array_unique($serials)) !== count($serials)) {
                        $errors[] = "Product " . ($item['description'] ?? "ID {$item['product_id']}") . " contains duplicate serial numbers in the input.";
                        continue;
                    }

                    $warehouseId = $item['warehouse_id'] ?? null;
                    if (!$warehouseId) {
                        $errors[] = "Product " . ($item['description'] ?? "ID {$item['product_id']}") . " requires a warehouse selection.";
                        continue;
                    }
                    ...
                    foreach ($serials as $serialNo) {
                        ...
                        } elseif ($serial->status !== 'available' || ($serial->service_job_id !== null && $serial->service_job_id != $serviceJobId)) {
    ...
    {
        // Release serial numbers currently linked to this service job that are available
        ...
        foreach ($items as $item) {
            ...
            if (isset($item['type']) && $item['type'] === 'part' && isset($item['product_id'])) {
                if ($trackingService->isProductTracked($item['product_id'])) {
                    $serials = $trackingService->parseSerials($item['serial_numbers'] ?? '');
                    if (!empty($serials)) {
                        ProductSerialNumber::whereIn('serial_no', $serials)
                            ->update(['service_job_id' => $job->id]);
                    }
                }
            }
        }
    ...
    protected function deductStockForJob(ServiceJob $job): void
    {
        ...
        foreach ($items as $row) {
            ...
            // Handle serial number updates on delivery/stock deduction
            $serials = ProductSerialNumber::where('service_job_id', $job->id)
                ->where('product_id', $row->product_id)
                ->where('status', 'available')
                ->when($row->product_variant_id, function ($q) use ($row) {
                    $q->where('variation_id', $row->product_variant_id);
            ...
            foreach ($serials as $serial) {
                ...
                // Create SerialNumberHistory log if it doesn't already exist
                ...
                if (!$historyExists) {
                    SerialNumberHistory::create([
                        ...
                        'notes' => 'Issued via Service Job delivery: ' . $job->Ref,
                        'created_by' => Auth::id(),
                    ]);
                }
            }
