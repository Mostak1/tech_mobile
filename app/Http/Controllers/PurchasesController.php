...
class PurchasesController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        if ($serialService->isSupported()) {
            $serialErrors = $serialService->validatePurchase($request['details']);
        ...
        \DB::transaction(function () use ($request, $serialService) {
            ...
            if ($serialService->isSupported()) {
                $persisted = PurchaseDetail::where('purchase_id', $order->id)
                    ->orderBy('id', 'asc')
                    ->get();
                $serialService->applyForPurchase($order, $data, $persisted);
            }
        }, 10);
    ...
    public function update(Request $request, $id)
    {
        ...
        $serialService = app(\App\Services\SerialTrackingService::class);
        if ($serialService->isSupported()) {
            $serialErrors = $serialService->validatePurchase($request['details'], $id);
            if (!empty($serialErrors)) {
                return response()->json([
                    'success' => false,
                    'message' => implode(' ', $serialErrors),
            ...
            */
            ...
            if (PurchaseReturn::where('purchase_id', $id)->where('deleted_at', '=', null)->exists()) {
                ...
                if ($serialService->isSupported()) {
                    ...
                    $serialService->applyForPurchase($current_Purchase, $alignedInput, $aligned);
                }
            }
    ...
    public function show(Request $request, $id)
    {
        ...
        foreach ($purchase['details'] as $detail) {
            ...
            $data['serial_numbers'] = \App\Models\ProductSerialNumber::where('purchase_line_id', $detail->id)
                ->pluck('serial_no')
                ->toArray();
