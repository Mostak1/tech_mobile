...
class TransferController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        if ($serialService->isSupported()) {
            $serialErrors = $serialService->validateTransfer($request['details'], (int) $request->transfer['from_warehouse']);
        ...
        \DB::transaction(function () use ($request, $serialService) {
            ...
            // Serial tracking: apply transfer serials
            if ($serialService->isSupported() && $shouldAffectStock && in_array($request->transfer['statut'], ['completed', 'sent'], true)) {
                $serialService->applyForTransfer(
                    $order,
                    array_values($data),
                    $persistedDetails
                );
    ...
    public function update(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($request, $id, $serialService) {
            $user = Auth::user();
            // New way: Check user's record_view field (user-level boolean)
            // Backward compatibility: If record_view is null, fall back to role permission check
            $view_records = $user->hasRecordView();
            ...
            // Serial tracking: reverse old transfer serials
            if ($serialService->isSupported()
                && $isApproved
                && in_array($current_Transfer->statut, ['completed', 'sent'], true)) {
                $serialService->reverseForTransferDetails($Old_Details, $current_Transfer->from_warehouse_id);
            }
            ...
            foreach ($data as $key => $product_detail) {
                ...
                if ($product_detail['no_unit'] !== 0) {
                    ...
                    $TransDetail['imei_number'] = $product_detail['serial_numbers'] ?? $product_detail['imei_number'] ?? null;
            ...
            // Serial tracking: re-apply transfer serials
            if ($serialService->isSupported()
                && $isApproved
                && in_array($Trans['statut'], ['completed', 'sent'], true)) {
                $alignedInput = [];
    ...
    public function destroy(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($id, $request) {
            ...
            // Serial tracking: reverse transfer serials on delete
            $serialService = app(\App\Services\SerialTrackingService::class);
            if ($serialService->isSupported()
                && $isApproved
                && in_array($current_Transfer->statut, ['completed', 'sent'], true)) {
    ...
    public function approve(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($request, $id) {
            ...
            // Serial tracking: apply transfer serials
            $serialService = app(\App\Services\SerialTrackingService::class);
            if ($serialService->isSupported() && in_array($transfer->statut, ['completed', 'sent'], true)) {
                $inputDetails = [];
                $persistedDetails = [];
                foreach ($transfer->details as $detail) {
                    $inputDetails[] = [
                        'product_id' => $detail->product_id,
                        'serial_numbers' => $detail->imei_number,
                    ];
                    $persistedDetails[] = $detail;
                }
                $serialService->applyForTransfer($transfer, $inputDetails, $persistedDetails);
            }
