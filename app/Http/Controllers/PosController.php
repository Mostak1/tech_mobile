...
class PosController extends BaseController
{
    ...
    public function CreatePOS(Request $request)
    {
        ...
        try {
            $sale = \DB::transaction(function () use ($request, $totalPaid, $saleUuid, $serialService) {
                $helpers = new helpers;
                $user = Auth::user();
                ...
                if ($serialService->isSupported()) {
                    $persistedDetails = SaleDetail::where('sale_id', $order->id)
                        ->orderBy('id')
                        ->get();
                    $serialService->applyForSale($order, array_values($data), $persistedDetails);
                }
    ...
    public function submit_sale_from_draft(Request $request)
    {
        ...
        if ($draft) {
            ...
            $sale = \DB::transaction(function () use ($request, $draft, $serialService) {
                $helpers = new helpers;
                $user = Auth::user();
                // New way: Check user's record_view field (user-level boolean)
                // Backward compatibility: If record_view is null, fall back to role permission check
