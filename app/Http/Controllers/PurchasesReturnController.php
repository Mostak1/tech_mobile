...
class PurchasesReturnController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        if ($serialService->isSupported()) {
            $serialErrors = $serialService->validatePurchaseReturn($request['details'], (int) $request->warehouse_id);
        ...
        \DB::transaction(function () use ($request, $serialService) {
            ...
            if ($order->statut == 'completed') {
                ...
                // Serial tracking: apply purchase return serials
                if ($serialService->isSupported()) {
                    $serialService->applyForPurchaseReturn(
                        $order,
                        array_values($data),
                        $persistedDetails
                    );
    ...
    public function update(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($request, $id, $serialService) {
            ...
            */
            ...
            // Serial tracking: reverse old purchase return serials
            if ($serialService->isSupported() && $current_PurchaseReturn->statut == 'completed') {
                $serialService->reverseForPurchaseReturnDetails($old_Return_Details);
            }
            ...
            // Serial tracking: re-apply purchase return serials
            if ($serialService->isSupported() && $request['statut'] == 'completed') {
                $alignedInput = [];
                $alignedPersisted = [];
                foreach ($New_Return_Details as $key => $product_detail) {
                    if (isset($newPersistedDetails[$key])) {
    ...
    public function destroy(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($id, $request) {
            ...
            */
            ...
            // Serial tracking: reverse purchase return serials on delete
            $serialService = app(\App\Services\SerialTrackingService::class);
            if ($serialService->isSupported() && $current_PurchaseReturn->statut == 'completed') {
                $serialService->reverseForPurchaseReturnDetails($old_Return_Details);
            }
