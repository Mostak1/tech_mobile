...
class ProductsController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            \DB::transaction(function () use ($request) {
                ...
                $Product->enable_serial_tracking = $request['enable_serial_tracking'] == 'true' || $request['enable_serial_tracking'] == 1 ? 1 : 0;
    ...
    public function Get_Products_Details(Request $request, $id)
    {
        ...
        $item['enable_serial_tracking'] = (bool) ($Product->enable_serial_tracking ?? false);
    ...
    // ------------ Get sales history of product -----------------\\
    public function sales_history(Request $request, $id)
    {
        $this->authorizeForUser($request->user('api'), 'view', Product::class);
        ...
        foreach ($paginated->items() as $detail) {
            ...
            $data[] = [
                ...
                'date' => $detail->sale->date,
                'ref' => $detail->sale->Ref,
                'client_name' => optional($detail->sale->client)->name,
                'warehouse_name' => optional($detail->sale->warehouse)->name,
    ...
    // ------------ Get purchases history of product -----------------\\
    public function purchases_history(Request $request, $id)
    {
        $this->authorizeForUser($request->user('api'), 'view', Product::class);
        ...
        $query = \App\Models\PurchaseDetail::with(['purchase.provider', 'purchase.warehouse', 'product.unit'])
            ->where('product_id', $id)
        ...
        if ($request->filled('search')) {
            $search = $request->search;
            $query->where(function ($q) use ($search) {
                $q->whereHas('purchase', function ($sq) use ($search) {
                    $sq->where('Ref', 'like', "%{$search}%")
        ...
        foreach ($paginated->items() as $detail) {
            $data[] = [
                ...
                'provider_name' => optional($detail->purchase->provider)->name,
                'warehouse_name' => optional($detail->purchase->warehouse)->name,
                'quantity' => $detail->quantity,
                'unit' => optional($detail->product->unit)->ShortName,
                'cost' => $detail->cost,
                'total' => $detail->total,
    ...
    public function available_serial_numbers(Request $request)
    {
        $this->authorizeForUser($request->user('api'), 'view', Product::class);
        ...
        $query = ProductSerialNumber::query()
            ...
            ->where('current_location_id', $validated['warehouse_id'])
            ->where('status', 'available')
            ->whereNull('service_job_id');

        if (! empty($validated['variant_id'])) {
            $query->where('variation_id', $validated['variant_id']);
        }

        if (! empty($validated['search'])) {
            $query->where('serial_no', 'like', '%'.$validated['search'].'%');
        }
    ...
    public function product_serial_numbers(Request $request, $id)
    {
        $this->authorizeForUser($request->user('api'), 'view', Product::class);

        $limit = $request->get('limit', 10);

        $query = ProductSerialNumber::with([
            'variation:id,name',
            'currentLocation:id,name',
            'currentCustomer:id,name,phone',
            'purchaseLine.purchase:id,Ref,date,provider_id',
            'purchaseLine.purchase.provider:id,name',
            'soldSellLine.sale:id,Ref,date,client_id',
            'soldSellLine.sale.client:id,name,phone',
            'serviceJob:id,Ref,status,client_id',
            'serviceJob.client:id,name,phone',
            'latestHistory',
        ])->where('product_id', $id);
        ...
        if ($request->filled('search')) {
            $query->where('serial_no', 'like', '%'.$request->get('search').'%');
        }

        $paginated = $query->orderBy('serial_no', 'asc')->paginate($limit);

        $serials = collect($paginated->items())->map(function (ProductSerialNumber $serial) {
            $purchase = optional($serial->purchaseLine)->purchase;
            $sale = optional($serial->soldSellLine)->sale;
            $serviceJob = $serial->serviceJob;
            $latestHistory = $serial->latestHistory;

            $warranty_expiry = null;
            if ($serial->soldSellLine && $serial->soldSellLine->warranty_date) {
                $warranty_expiry = $serial->soldSellLine->warranty_date;
            }
            ...
            return [
                ...
                'warehouse' => optional($serial->currentLocation)->name,
                'customer' => optional($serial->currentCustomer)->name
                    ?: optional(optional($sale)->client)->name
                    ?: optional(optional($serviceJob)->client)->name,
                'customer_phone' => optional($serial->currentCustomer)->phone
                    ?: optional(optional($sale)->client)->phone
                    ?: optional(optional($serviceJob)->client)->phone,
                'purchase_ref' => optional($purchase)->Ref,
                'purchase_date' => optional($purchase)->date,
                'supplier' => optional(optional($purchase)->provider)->name,
                'sale_ref' => optional($sale)->Ref,
                'sale_date' => optional($sale)->date,
                'service_ref' => optional($serviceJob)->Ref,
                'service_status' => optional($serviceJob)->status,
                'warranty_expiry' => $warranty_expiry,
                'last_activity' => $latestHistory ? [
                    'type' => $latestHistory->type,
                    'notes' => $latestHistory->notes,
                    'created_at' => $latestHistory->created_at->format('Y-m-d H:i'),
                ] : null,
                'updated_at' => optional($serial->updated_at)->format('Y-m-d H:i'),
            ];
        });
    ...
    public function serial_number_history(Request $request, $id)
    {
        $this->authorizeForUser($request->user('api'), 'view', Product::class);

        $serial = ProductSerialNumber::findOrFail($id);

        $histories = \App\Models\SerialNumberHistory::with([
        ...
        ->orderBy('id', 'asc')
        ->get()
        ->map(function (\App\Models\SerialNumberHistory $history) {
            return [
                'id' => $history->id,
                'type' => $history->type,
                'reference_id' => $history->reference_id,
                'from_location' => optional($history->fromLocation)->name,
                'to_location' => optional($history->toLocation)->name,
                'customer' => optional($history->customer)->name,
                'customer_phone' => optional($history->customer)->phone,
                'notes' => $history->notes,
                'created_by' => optional($history->creator)->username,
    ...
    public function show_product_data($id, $variant_id, $warehouse_id = null)
    {
        ...
        $item['enable_serial_tracking'] = (bool) ($Product_data['enable_serial_tracking'] ?? false);
    ...
    public function edit(Request $request, $id)
    {
        ...
        $item['enable_serial_tracking'] = $Product->enable_serial_tracking ? true : false;
