...
class DamageController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        if ($serialService->isSupported()) {
            $serialErrors = $serialService->validateDamage($request['details'], $request->warehouse_id);
        ...
        \DB::transaction(function () use ($request, $serialService) {
            ...
            // Serial tracking
            if ($serialService->isSupported()) {
                $serialService->applyForDamage($order, array_values($data), $persistedDetails);
            }
        }, 10);
    ...
    public function update(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($request, $id, $current_damage, $serialService) {
            ...
            // Serial tracking: reverse existing serial numbers
            if ($serialService->isSupported()) {
                $serialService->reverseForDamageDetails($old_details);
            }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($id, $request) {
            ...
            // Serial tracking: reverse serial numbers
            $serialService = app(\App\Services\SerialTrackingService::class);
            if ($serialService->isSupported()) {
                $serialService->reverseForDamageDetails($old_details);
            }
    ...
    public function edit(Request $request, $id)
    {
        ...
        // Fetch serial numbers associated with this damage
        $histories = \App\Models\SerialNumberHistory::where('type', 'damage')
            ->where('reference_id', $Damage_data->id)
            ->with('serialNumber')
            ->get();
